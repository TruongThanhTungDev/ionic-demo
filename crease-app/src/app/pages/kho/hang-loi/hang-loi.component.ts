...
import { XulyPhieuHangLoiComponent } from 'src/app/shared/popup/them-sua-kho/them-sua-phieu-hang-loi/xu-ly-phieu-hang-loi.component';
@Component({
  selector: 'hangloi-component',
  templateUrl: './hang-loi.component.html',
})
export class HangLoiComponent implements OnInit {
  @Input() data: any;
  @Input() title: any;
  @Input() type: any;
  ...
  public filterData() {
    ...
    if (this.FtType) {
      filter.push(`type==${this.FtType}`);
    } else {
      filter.push(`type =in= (4,5)`);
    }
    return filter.join(';');
  }
  ...
  async addPhieuHangLoi() {
    const modal = await this.modal.create({
      component: XulyPhieuHangLoiComponent,
      componentProps: {
        title: 'Hàng lỗi',
        data: null,
        type: 'add',
  ...
  async editInfoPhieuHangLoi(item: any) {
    const modal = await this.modal.create({
      component: XulyPhieuHangLoiComponent,
      componentProps: {
        title: 'Hàng lỗi',
